...
class PatchCreditCardsBody(BaseModel):
    """Body for PATCH /simulation/{session_id}/credit-cards."""
    credit_cards: List[CreditCardConfig] = []


@router.patch("/simulation/{session_id}/credit-cards")
async def patch_session_credit_cards(session_id: str, body: PatchCreditCardsBody) -> Dict:
    """
    Update credit cards for an existing session (live update).
    Replaces all cards for the session.
    """
    ...
    ok = session_manager.update_session_credit_cards(session_id, body.credit_cards)
    if not ok:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update credit cards")
    return {"ok": True, "session_id": session_id, "credit_cards_count": len(body.credit_cards)}
