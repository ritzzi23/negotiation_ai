...
from ..services.deal_calculator import compute_deal_context
...
class SessionManager:
    ...
    def create_session(
        ...
        with get_db() as db:
            ...
            # Persist credit cards for this session (Place 2: per-buyer cards)
            for card in request.credit_cards or []:
                card_id = card.card_id or str(uuid.uuid4())
                rewards_json = json.dumps(
                    [{"category": r.category, "cashback_pct": r.cashback_pct} for r in card.rewards]
                ) if card.rewards else None
                vendor_offers_json = json.dumps(
                    [
                        {"vendor_keyword": v.vendor_keyword, "discount_pct": v.discount_pct, "max_discount": getattr(v, "max_discount", 0)}
                        for v in card.vendor_offers
                    ]
                ) if card.vendor_offers else None
                db.add(CreditCardRecord(
                    id=str(uuid.uuid4()),
                    session_id=session_id,
                    card_id=card_id,
                    ...
                    vendor_offers_json=vendor_offers_json,
    ...
    def update_session_credit_cards(self, session_id: str, credit_cards: list) -> bool:
        """
        Update credit cards for an existing session (replace all).
        ...
        with get_db() as db:
            ...
            if not session:
                return False
            db.query(CreditCardRecord).filter(CreditCardRecord.session_id == session_id).delete()
            for card in credit_cards or []:
                card_id = getattr(card, "card_id", None) or str(uuid.uuid4())
                rewards = getattr(card, "rewards", []) or []
                vendor_offers = getattr(card, "vendor_offers", []) or []
                rewards_json = json.dumps(
                    [{"category": r.category, "cashback_pct": r.cashback_pct} for r in rewards]
                ) if rewards else None
                vendor_offers_json = json.dumps(
                    [
                        {"vendor_keyword": v.vendor_keyword, "discount_pct": v.discount_pct, "max_discount": getattr(v, "max_discount", 0)}
                        for v in vendor_offers
                    ]
                ) if vendor_offers else None
                db.add(CreditCardRecord(
                    id=str(uuid.uuid4()),
                    session_id=session_id,
                    card_id=card_id,
                    ...
                    rewards_json=rewards_json,
                    vendor_offers_json=vendor_offers_json,
                    annual_fee=float(getattr(card, "annual_fee", 0) or 0),
                ))
            db.commit()
            logger.info(f"Updated credit cards for session {session_id}: {len(credit_cards or [])} cards")
            return True
        ...
        for seller_orm in sellers_orm:
            ...
            sellers.append(SellerModel(
                ...
                profile=SellerProfile(
                    ...
                    strategy=getattr(seller_orm, 'strategy', 'firm_pricing')
                ),
                inventory=inventory
            ))
    ...
    def finalize_run(
        ...
        with get_db() as db:
            ...
            # Credit card fields: compute from Deal Calculator when we have a deal
            recommended_card = None
            card_savings = None
            effective_total = None
            if decision_type == "deal" and selected_seller_id and final_price_per_unit and quantity:
                buyer_item = db.query(BuyerItem).filter(BuyerItem.id == run.buyer_item_id).first()
                seller_orm = db.query(Seller).filter(Seller.id == selected_seller_id).first()
                if buyer_item and seller_orm:
                    inv = db.query(SellerInventory).filter(
                        SellerInventory.seller_id == selected_seller_id,
                        SellerInventory.item_id == buyer_item.item_id,
                    ).first()
                    if inv:
                        wallet = get_wallet_for_session(run.session_id)
                        ctx = compute_deal_context(
                            price_per_unit=final_price_per_unit,
                            quantity=quantity,
                            item_name=buyer_item.item_name,
                            seller_name=seller_orm.name,
                            seller_cost_per_unit=inv.cost_price,
                            wallet=wallet,
                        )
