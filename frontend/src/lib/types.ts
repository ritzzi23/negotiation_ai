import { SellerPriority, SpeakingStyle, SellerStrategy, NegotiationStatus, SessionStatus } from './constants';
...
export interface CreditCardVendorOffer {
  vendor_keyword: string;
  discount_pct: number;
  max_discount: number;
}
...
export interface CreditCardConfig {
  ...
  vendor_offers: CreditCardVendorOffer[];
  annual_fee: number;
}
...
export interface DealExplanation {
  ...
  failed_sellers: { seller_id: string; seller_name: string; status: string; reason: string; rounds_taken: number }[];
  summary: string;
  credit_card_tip: string;
}
...
export interface BuyerDecision {
  ...
  total_cost?: number;
  timestamp: string;
  /** Effective total after card rewards (when available). */
  effective_total?: number;
  /** Recommended card name for this purchase. */
  recommended_card?: string;
  /** Savings from using the recommended card. */
  card_savings?: number;
}
...
export type NegotiationEvent =
  ...
  | { type: 'decision'; decision?: 'accept' | 'reject' | 'no_deal'; chosen_seller_id?: string; chosen_seller_name?: string; final_price?: number; final_quantity?: number; total_cost?: number; effective_total?: number; recommended_card?: string; card_savings?: number; reason?: string; round?: number; timestamp: string }
