    ...
    <div className="min-h-screen bg-white text-neutral-900">
      ...
      <header className="bg-white px-4 py-3 flex items-center justify-between border-b border-neutral-200">
        <div className="flex items-center space-x-2">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center border border-neutral-300 bg-white">
            <svg className="w-5 h-5 text-neutral-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <span className="text-lg font-semibold">DealForge</span>
          <span className="text-[10px] text-neutral-600 bg-neutral-100 px-2 py-0.5 rounded-full border border-neutral-200">Mobile</span>
        </div>
        {paired && (
          <span className="flex items-center text-xs text-secondary-600">
      ...
      {!paired && (
        <div className="flex flex-col items-center justify-center p-8 min-h-[calc(100vh-56px)]">
          <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center mb-6 border border-neutral-200">
          ...
          <p className="text-neutral-500 text-sm text-center mb-8 max-w-xs">
          ...
          <input
            ...
            className="w-full max-w-sm bg-white border border-neutral-300 rounded-xl px-4 py-3 text-neutral-900 placeholder-neutral-400 mb-4 text-center font-mono text-sm"
          />
          <button
            onClick={handlePair}
            disabled={!sessionId.trim()}
            className="w-full max-w-sm bg-primary-600 text-white rounded-xl py-3 font-semibold disabled:opacity-50 transition-all hover:-translate-y-0.5"
          >
            Connect
          </button>
        ...
        <div className="pb-20">
          ...
          {activeTab === 'scan' && (
            <div className="p-4 space-y-4">
              ...
              <p className="text-sm text-neutral-500">Paste a product link or enter details to send to your PC agent</p>
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-white border border-neutral-300 rounded-xl px-4 py-3 text-neutral-900 placeholder-neutral-400"
                />
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    className="bg-white border border-neutral-300 rounded-xl px-4 py-3 text-neutral-900 placeholder-neutral-400"
                  />
                  <input
                    type="number"
                    placeholder="Qty"
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full bg-white border border-neutral-300 rounded-xl px-4 py-3 text-neutral-900 placeholder-neutral-400 resize-none"
                />
                <button
                  onClick={handleSubmitConstraint}
                  disabled={submitting || !constraint.item_name || !constraint.max_budget}
                  className="w-full bg-primary-600 text-white rounded-xl py-3.5 font-semibold disabled:opacity-50 transition-all flex items-center justify-center hover:-translate-y-0.5"
                >
                  {submitting ? (
                ...
                {submitted && (
                  <div className="bg-secondary-50 border border-secondary-200 rounded-xl p-3 text-secondary-700 text-sm text-center">
            ...
            <div className="p-4 space-y-4">
              ...
              {notifications.length === 0 ? (
                <div className="text-center py-12 text-neutral-500">
                  <div className="text-sm uppercase tracking-wide text-neutral-400 mb-2">Pending</div>
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          n.status === 'deal' ? 'bg-secondary-100 text-secondary-700' :
                          n.status === 'in_progress' ? 'bg-primary-100 text-primary-700' :
                      ...
                      {n.final_price && (
                        <div className="text-2xl font-bold text-secondary-700 mb-1">${n.final_price.toFixed(2)}</div>
                      )}
                      {n.winner_seller && (
                        <p className="text-sm text-neutral-600">from {n.winner_seller}</p>
                      )}
                      {n.card_tip && (
                        <p className="text-xs text-neutral-600 mt-2">{n.card_tip}</p>
                      )}
                      <p className="text-xs text-neutral-500 mt-1">{n.summary}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {activeTab === 'wallet' && (
            <div className="p-4 space-y-4">
              ...
              <p className="text-sm text-neutral-500">Your cards are factored into every deal analysis</p>
              {[
                { name: 'Chase Sapphire Preferred', issuer: 'Chase', color: 'border-neutral-200 bg-white', rewards: '5% travel, 3% dining, 2% electronics' },
                { name: 'Amex Blue Cash Preferred', issuer: 'Amex', color: 'border-neutral-200 bg-white', rewards: '6% groceries, 3% electronics' },
                { name: 'Discover it Cash Back', issuer: 'Discover', color: 'border-neutral-200 bg-white', rewards: '5% electronics (rotating), 1% everything' },
              ].map((card, i) => (
                <div key={i} className={`rounded-2xl p-5 shadow-sm border ${card.color}`}>
                  <div className="flex justify-between items-start mb-4">
                    <span className="text-xs text-neutral-500 font-medium">{card.issuer}</span>
                    <svg className="w-8 h-8 text-neutral-300" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M20 4H4c-1.11 0-1.99.89-1.99 2L2 18c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z"/>
                    </svg>
                  </div>
                  <div className="text-lg font-semibold text-neutral-900 mb-1">{card.name}</div>
                  <div className="text-xs text-neutral-500">{card.rewards}</div>
                </div>
              ))}
            </div>
          )}
          ...
          {/* Bottom Tab Bar */}
          <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 flex">
            {[
              ...
              <button
                ...
                className={`flex-1 py-3 flex flex-col items-center ${
                  activeTab === tab.id ? 'text-primary-700' : 'text-neutral-400'
                }`}
              >
